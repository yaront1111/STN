...
int main() {
    ...
    // Initialize components with REAL EGM2008 data
    GravityGradientProvider gravity_model;
    if (!gravity_model.loadEGM2020("data/egm2008/egm2008_n360.dat")) {
        std::cerr << "ERROR: Failed to load EGM2008 data\n";
        return 1;
    }
    ...
    // HIERARCHICAL SEARCH: Two-stage for speed
    GravityMapMatcher::Config matcher_config;
    matcher_config.signature_length = 100;        // 10 seconds at 10 Hz - more unique signature
    matcher_config.correlation_threshold = 0.50;  // Even lower for first detection
    matcher_config.search_radius_m = 5000;        // Reduced to 5km for speed
    matcher_config.grid_resolution_m = 500;       // Very coarse for initial search (500m)
    matcher_config.min_measurements = 50;         // 5 seconds minimum for robust matching
    ...
    std::cout << "Running 1-minute test with Figure-8 maneuvers...\n";
    std::cout << "─────────────────────────────────────────────\n";
    ...
    for (int i = 0; i < steps; i++) {
        ...
        if (i % 10 == 0) {
            ...
            if (tensor.T.allFinite() && tensor.T.norm() < 100.0) {
                ...
                std::cerr << "WARNING: Invalid gradient tensor, norm=" << tensor.T.norm() 
                          << ", trace=" << tensor.T.trace() << "\n";
                // Print first few elements for debugging
                if (i < 10) {
                    std::cerr << "  T_xx=" << tensor.T(0,0) << ", T_yy=" << tensor.T(1,1) 
                              << ", T_zz=" << tensor.T(2,2) << "\n";
                }
            }
        ...
        if (current_time > 5.0 && current_time - perf.last_match_time >= 3.0 && 
            ...
            // std::cout << "\n[t=" << current_time << "s] Attempting map match...\n";
